 ...
 * @version v1.0 2019年1月19日
 * @since
 */
public abstract class CheckSums {

    /**
     * Die berechnung der CRC16
     * 
     * @param str
     * @return
     */
     ...
     * @return
     */
    public static long crc16(byte[] bys) {
        return PureJavaCrc16.crc16(bys);
    }

    /**
     * Die berechnung der CRC32
     * 
     * @param str
     * @return
     */
     ...
     * @return
     */
    public static long crc32(byte[] bys) {
    ...
    /**
     * A pure-java implementation of the CRC16 checksum that uses the same
     * polynomial as the built-in native CRC16.
     * 
     * @author James Wong James Wong <jameswong1376@gmail.com>
     * @version v1.0 2019年1月19日
     * @since
     */
    public static class PureJavaCrc16 {

        final private static byte[] CRC16_TAB_HI = { (byte) 0x00, (byte) 0xC1, (byte) 0x81, (byte) 0x40, (byte) 0x01, (byte) 0xC0,
        ...
        final private static byte[] CRC16_TAB_LO = { (byte) 0x00, (byte) 0xC0, (byte) 0xC1, (byte) 0x01, (byte) 0xC3, (byte) 0x03,
        ...
        public static int crc16(byte[] data, int offset, int len) {
            return crc16(data, offset, len, 0xffff);
        }
        ...
        public static int crc16(byte[] data, int offset, int len, int preval) {
            ...
            for (int i = 0; i < len; ++i) {
                ...
                ucCRCLo = ucCRCHi ^ CRC16_TAB_HI[iIndex];
                ucCRCHi = CRC16_TAB_LO[iIndex];
            }
    ...
    /**
     * <font color=red> Compared with the environment of oracle/jdk8, the speed
     * of pro-test oracle/jdk8 is slightly slower than that of oracle/jdk8
     * crc32. </font></br>
     * 
     ...
     * 
     * This is to avoid the JNI overhead for certain uses of Checksumming where
     * many small pieces of data are checksummed in succession.
     * 
     * The current version is ~10x to 1.8x as fast as Sun's native
     * java.util.zip.CRC32 in Java 1.6 </br>
     * 
     * <b>Javascript - version crc32 algorithmus implementiert:</b>
     * 
     * <pre>
     ...
     *
     *  // Testing
     *  console.log(crc32("some string"));
     * </pre>
     * 
     * @see java.util.zip.CRC32
     */
    public static class PureJavaCrc32 implements Checksum {
