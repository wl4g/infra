...
package com.wl4g.infra.common.web.server;
...
/**
 * The {@code SimpleHTTPServer} class implements a light-weight HTTP server.
 * <p>
 ...
 * functionality (this is termed "conditionally compliant" in the RFC). In fact,
 * a couple of bugs in the RFC itself were discovered (and fixed) during the
 * development of this server.
 * <p>
 ...
 * <b>Use Cases</b>
 * <p>
 * Being a lightweight, standalone, easily embeddable and tiny-footprint server,
 * it is well-suited for
 * <ul>
 * <li>Resource-constrained environments such as embedded devices. For really
 * extreme constraints, you can easily remove unneeded functionality to make it
 * even smaller (and use the -Dstripped maven build option to strip away debug
 * info, license, etc.)</li>
 ...
 * <li>Embedding a web console into any headless application for administration,
 * monitoring, or a full portable GUI.</li>
 ...
 * <b>Getting Started</b>
 * <p>
 * For an example and a good starting point for learning how to use the API, see
 * the {@link #main main} method at the bottom of the file, and follow the code
 * into the API from there. Alternatively, you can just browse through the
...
public class SimpleHTTPServer {
    ...
    public static class ChunkedInputStream extends LimitedInputStream {
        ...
        protected static long parseChunkSize(String line) throws IllegalArgumentException {
            ...
            line = pos < 0 ? line : line.substring(0, pos); // ignore params, if
                                                            // any
            try {
    ...
    /**
     * The {@code ResponseOutputStream} encompasses a single response over a
     ...
     * "multipart/*" content type (see RFC 2046), providing the underlying data
     * of its various parts.
     * <p>
     * The {@code InputStream} methods (e.g. {@link #read}) relate only to the
     * current part, and the {@link #nextPart} method advances to the beginning
     * of the next part.
     */
    public static class MultipartInputStream extends FilterInputStream {
        ...
        protected int state; // initial, started data, start boundary, EOS, last
                             // boundary, epilogue
        ...
        public int read(byte[] b, int off, int len) throws IOException {
            ...
            System.arraycopy(buf, head, b, off, len); // throws IOOBE as
                                                      // necessary
            head += len;
            return len;
        }
        ...
        /**
         * Advances the stream position to the beginning of the next part. Data
         * read before calling this method for the first time is the preamble,
        ...
        protected boolean fill() throws IOException {
            ...
            if (head != tail) // remember that if we continue, head == tail
                              // below
                return true;
            // if there's no more room, shift extra unread data to beginning of
            // buffer
            if (tail > buf.length - 256) { // max boundary + whitespace
                                           // supported size
                System.arraycopy(buf, tail, buf, 0, end -= tail);
                head = tail = 0;
            }
            // read more data and look for boundary (or potential partial
            // boundary)
            int read;
            do {
                ...
                findBoundary(); // updates tail and length to next potential
                                // boundary
                // if we found a partial boundary with no data before it, we
                // must
            ...
            if ((state & 6) == 4 // EOS but no start boundary found
                    || len == 0 && ((state & 0xFC) == 4 // EOS but no last and
                                                        // no more boundaries
                            || read == 0 && tail == head)) // boundary longer
                                                           // than buffer
        ...
        protected void findBoundary() throws IOException {
            ...
            len = 0;
            int off = tail - ((state & 1) != 0 || buf[0] != '-' ? 0 : 2); // skip
                                                                          // initial
                                                                          // CRLF?
            for (int end = this.end; tail < end; tail++, off = tail) {
                int j = tail; // end of potential boundary
                // try to match boundary value (leading CRLF is optional at
                // first boundary)
                while (j < end && j - off < boundary.length && buf[j] == boundary[j - off])
                    j++;
                // return potential partial boundary which is cut off at end of
                // current data
                if (j + 1 >= end) // at least two more chars needed for full
                                  // boundary (CRLF or --)
                    return;
                // if we found the boundary value, expand selection to include
                // full line
                if (j - off == boundary.length) {
                    ...
                    while (j < end && (buf[j] == ' ' || buf[j] == '\t'))
                        j++;
                    // check for CRLF (required, except in last boundary with no
                    // epilogue)
                    if (j + 1 < end && buf[j] == '\r' && buf[j + 1] == '\n') // found
                                                                             // CRLF
                        len = j - tail + 2; // including optional whitespace and
                                            // CRLF
                    else if (j + 1 < end || (state & 4) != 0 && j + 1 == end) // should
                                                                              // have
                                                                              // found
                                                                              // or
                                                                              // never
                    ...
                    else if ((state & 4) != 0) // last boundary with no CRLF at
     ...
     * attributes method="post" and enctype="multipart/form-data", and an action
     * URL of your choice, for example action="/upload". This form can be served
     * normally like any other resource, e.g. from an HTML file on disk.
     * <li>Add a context handler for the action path ("/upload" in this
     * example), using either the explicit {@link VirtualHost#addContext} method
     * or the {@link Context} annotation.
     * <li>In the context handler implementation, construct a
     * {@code MultipartIterator} from the client {@code Request}.
     * <li>Iterate over the form {@link Part}s, processing each named field as
     * appropriate - for the file input field, read the uploaded file using the
     * body input stream.
     * </ol>
     */
    public static class MultipartIterator implements Iterator<MultipartIterator.Part> {
        ...
        public static class Part {
            ...
            /**
             * Returns the part's filename (original filename entered in file
             * form field).
             *
            ...
            /***
             * Returns the part's body as a string. If the part headers do not
             * specify a charset, UTF-8 is used.
             *
    ...
    /**
     * A {@code ContextHandler} serves the content of resources within a
     * context.
     *
    ...
    public static class MethodContextHandler implements ContextHandler {
        ...
        public MethodContextHandler(Method m, Object obj) throws IllegalArgumentException {
            ...
            if (params.length != 2 || !Request.class.isAssignableFrom(params[0]) || !Response.class.isAssignableFrom(params[1])
    ...
    public class Request {
        ...
        /**
         * Returns the underlying socket, which can be used to retrieve
        ...
        public void setPath(String path) {
            try {
                uri = new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost(), uri.getPort(), trimDuplicates(path, '/'),
                        uri.getQuery(), uri.getFragment());
                context = null; // clear cached context so it will be
                                // recalculated
        ...
        public VirtualHost getVirtualHost() {
            return host != null ? host
                    : (host = SimpleHTTPServer.this.getVirtualHost(getBaseURL().getHost())) != null ? host
                            : (host = SimpleHTTPServer.this.getVirtualHost(null));
        }
    ...
    public class Response implements Closeable {
        ...
        public OutputStream getBody() throws IOException {
            ...
            encodedOut = new ResponseOutputStream(out); // leaves underlying
                                                        // stream open when
                                                        // closed
        ...
        public void close() throws IOException {
            state = -1; // closed
            if (encodedOut != null)
                encodedOut.close(); // close all chained streams (except the
                                    // underlying one)
        ...
        public void sendHeaders(int status, long length, long lastModified, String etag, String contentType, long[] range)
            ...
            if (!headers.contains("Content-Length") && !headers.contains("Transfer-Encoding")) {
                ...
                if (compression != null && (length < 0 || length > 300) && isCompressible(ct) && modern) {
                    // compressed data is always unknown length
            ...
            if (req != null && "close".equalsIgnoreCase(req.getHeaders().get("Connection")) && !headers.contains("Connection")) {
    ...
    protected class SocketHandlerThread extends Thread {
        ...
        public void run() {
            ...
            try {
                ServerSocket serv = SimpleHTTPServer.this.serv; // keep local to
                                                                // avoid
                // NPE when stopped
                            ...
                            try {
                                try {
                                    ...
                                    try {
                                        ...
                                        // (except SSL socket which doesn't
    ...
    /**
     * Constructs an SimpleHTTPServer which can accept connections on the given
     * port. Note: the {@link #start()} method must be called to start accepting
     * connections.
     *
     * @param port
    ...
    /**
     * Constructs an SimpleHTTPServer which can accept connections on the
     * default HTTP port 80. Note: the {@link #start()} method must be called to
    ...
    /**
     * Sets the factory used to create the server socket. If null or not set,
     * the default {@link ServerSocketFactory#getDefault()} is used. For secure
     * sockets (HTTPS), use an SSLServerSocketFactory instance. The port should
     ...
     * configured {@link #setServerSocketFactory ServerSocketFactory} and
     * {@link #setPort port}.
     * <p>
    ...
    public synchronized void start() throws IOException {
        if (serv != null)
            return;
        if (serverSocketFactory == null) // assign default server socket factory
                                         // if needed
            serverSocketFactory = ServerSocketFactory.getDefault(); // plain
                                                                    // sockets
    ...
    /**
     * Stops this server. If it is already stopped, does nothing. Note that if
     * an {@link #setExecutor Executor} was set, it must be closed separately.
     */
            ...
            try {
                ...
                if (req == null) { // error reading request
                    if (t instanceof IOException && t.getMessage().contains("missing request line")) {
                        // we're not in the middle of a transaction - so just
                        // disconnect
                        break;
                    }
                ...
                // if headers were not already sent, we can send an error
                // response
        ...
        if (version.equals("HTTP/1.1")) {
            ...
            if (expect != null) {
                if (expect.equalsIgnoreCase("100-continue")) {
                    Response _resp = new Response(resp.getOutputStream());
                    _resp.sendHeaders(100);
                    _resp.getOutputStream().flush();
                } else {
    ...
    protected void serve(Request req, Response resp) throws IOException {
        ...
        /**
         * <pre>
         *  Refer to nginx access log example:
         *  43.154.80.231 - - [12/May/2022:11:35:41 +0800] "GET / HTTP/1.1" 301 1234 "-" "python-requests/2.23.0" "-"
         * </pre>
         */
        System.out.println(format("%s - [%s] \"%s %s %s\" %s %s - \"%s\" - %sms", req.getSocket().getRemoteSocketAddress(),
                new Date(), req.getMethod(), req.getPath(), req.getVersion(), status, resp.getHeaders().get("Content-Length"),
                req.getHeaders().get("User-Agent"), (currentTimeMillis() - begin)));
    }
     ...
     * first occurrence of the given delimiter byte, in the given encoding. If
    ...
    public static String readToken(InputStream in, int delim, String enc, int maxLength) throws IOException {
        ...
        while ((b = in.read()) != -1 && b != delim) {
            if (count == len) { // expand buffer
                ...
                len = len > 0 ? 2 * len : 256; // start small, double each
                                               // expansion
    ...
    public static int getConditionalStatus(Request req, long lastModified, String etag) {
        ...
        if (header != null) {
            if (match(false, splitElements(header, false), etag)) // RFC7232#3.2:
                                                                  // use weak
                                                                  // matching
    ...
    public static int serveFile(File base, String context, Request req, Response resp) throws IOException {
        ...
        if (!file.exists() || file.isHidden() || file.getName().startsWith(".")) {
            ...
            return 404; // non-directory ending with slash (File constructor
                        // removed it)
        } else {
    ...
    public static String createIndex(File dir, String path) {
        ...
        try (Formatter f = new Formatter(Locale.US);) {
            f.format("<!DOCTYPE html>%n" + "<html><head><title>Index of %s</title></head>%n" + "<body><h1>Index of %s</h1>%n"
                    + "<pre> Name%" + (w - 5) + "s Last modified      Size<hr>", path, path, "");
            ...
            for (File file : dir.listFiles()) {
                try {
                    ...
                    if (!file.isHidden() && !name.startsWith("."))
                        f.format(" <a href=\"%s\">%s</a>%-" + (w - name.length()) + "s&#8206;%td-%<tb-%<tY %<tR%6s%n", link, name,
        ...
        try {
            if (args.length == 0) {
                System.err.printf("Usage: java [-options] %s <directory> [port]%n"
                        ...
                        + "-Djavax.net.ssl.keyStorePassword, etc.%n", SimpleHTTPServer.class.getName());
                return;
            }
            ...
            final int port = args.length < 2 ? 80 : (int) parseULong(args[1], 10);
            // set up server
            ...
            final SimpleHTTPServer server = new SimpleHTTPServer(port);
            ...
            // default host
            final VirtualHost host = server.getVirtualHost(null);
            ...
            System.out.println("SimpleHTTPServer is listening on port " + port);
        } catch (Exception e) {
