 ...
 * {@link Iterator} to iterate over the keys. This class is not thread safe.
 *
 * @param <K>
 *            the type of the iterated objects, which are keys in RocksDB.
 * @see Copy of
 *      https://github1s.com/apache/flink/blob/release-1.15.1/flink-state-backends/src/main/java/org/apache/flink/contrib/streaming/state/RocksDBKeysIterator.java
 */
public class RocksStateKeysIterator<K> extends AbstractRocksStateKeysIterator<K> implements Iterator<K> {
    ...
    public RocksStateKeysIterator(@Nonnull RocksIteratorWrapper iterator, @Nonnull String state,
            @Nonnull TypeSerializer<K> keySerializer, int keyGroupPrefixBytes, boolean ambiguousKeyPossible,
    ...
    public boolean hasNext() {
        try {
            ...
            throw new RuntimeException("Failed to access state [" + state + "]", e);
        }
        return nextKey != null;
    }
