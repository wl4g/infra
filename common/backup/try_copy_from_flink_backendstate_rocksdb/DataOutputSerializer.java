...
public class DataOutputSerializer implements DataOutputView, MemorySegmentWritable {
    ...
    /**
     * Gets a reference to the internal byte buffer. This buffer may be larger
     * than the actual serialized data. Only the bytes from zero to
     * {@link #length()} are valid. The buffer will also be overwritten with the
     * next write calls.
     *
     * <p>
     * This method is useful when trying to avid byte copies, but should be used
     * carefully.
     *
     ...
     * serialized so far. The returned buffer is an exclusive copy and can be
     * safely used without being overwritten by future write calls to this
     * serializer.
     *
     * <p>
     * This method is equivalent to
     * {@code Arrays.copyOf(getSharedBuffer(), length());}
     *
        ...
        try {
            ...
            if (newLen > this.buffer.length + minCapacityAdd) {
                ...
                try {
                    ...
                    // still not possible. give an informative exception message
                    // that reports the
                    // size
                    throw new IOException(
                            "Failed to serialize element. Serialized size (> " + newLen + " bytes) exceeds JVM heap space", ee);
                }
            } else {
                throw new IOException(
                        "Failed to serialize element. Serialized size (> " + newLen + " bytes) exceeds JVM heap space", e);
            }
        }
    ...
    public void setPosition(int position) {
        // @formatter:off
//        org.apache.flink.util.Preconditions.checkArgument(position >= 0 && position <= this.position, "Position out of bounds.");
        // @formatter:on
        isTrue(position >= 0 && position <= this.position, "Position out of bounds.");
