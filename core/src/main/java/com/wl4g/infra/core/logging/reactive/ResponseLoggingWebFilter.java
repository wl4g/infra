 ...
 */
@Slf4j
public class ResponseLoggingWebFilter extends BaseLoggingWebFilter {

    public ResponseLoggingWebFilter(LoggingMessageProperties loggingConfig, Environment environment) {
    ...
    protected Mono<Void> doFilterInternal(
        ...
        Long startTime = exchange.getAttribute(LoggingMessageUtil.KEY_START_TIME);
            ...
            if (transformCount.incrementAndGet() <= 1) {
                ...
                if (log6_10) {
                    ...
                    httpHeaders.forEach((headerName, headerValue) -> {
                        if (log8_10 || LoggingMessageUtil.LOG_GENERIC_HEADERS.stream()
                                .anyMatch(h -> containsIgnoreCase(h, headerName))) {
                            responseLog.append("\n{}: {}");
                ...
                // binary.
                if (LoggingMessageUtil.isDownloadStreamMedia(headers.getContentType())) {
                    ...
                    // directly.
                    boolean processBodyIfNeed = log9_10
                            && LoggingMessageUtil.isCompatibleWithPlainBody(response.getHeaders().getContentType());
                    // Print response body.
        ...
        return chain.filter(exchange.mutate().response(newRespnose).build()).doFinally(signal -> {
            ...
            if (!addedResponseHeaders.get() && log6_10) {
                ...
                httpHeaders.forEach((headerName, headerValue) -> {
                    if (log8_10
                            || LoggingMessageUtil.LOG_GENERIC_HEADERS.stream().anyMatch(h -> containsIgnoreCase(h, headerName))) {
