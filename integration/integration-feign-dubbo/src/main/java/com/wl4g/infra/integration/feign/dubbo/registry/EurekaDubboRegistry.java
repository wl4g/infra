 ...
 */
package com.wl4g.infra.integration.feign.springcloud.dubbo.registry;
...
public class EurekaDubboRegistry extends FailbackRegistry {
	...
	private EurekaEventListener eurekaEventListener = (event) -> {
		if (event instanceof CacheRefreshedEvent) {
			...
			eurekaClient.getApplications().getRegisteredApplications().stream()
					.forEach(application -> application.getInstances().stream().forEach(instanceInfo -> {
	...
	private void addOrDeleteMetadata(URL url, String category, boolean isAdd) {
		...
		try {
			TreeSet<String> values = StringUtils.isEmpty(metadataCategory) ? new TreeSet<>()
					: objectMapper.readValue(metadataCategory, stringListTypeReference);
			if (isAdd) {
