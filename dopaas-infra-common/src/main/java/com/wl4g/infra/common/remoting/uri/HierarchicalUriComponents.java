...
final class HierarchicalUriComponents extends UriComponents {
	...
	private static final MultiValueMap<String, String> EMPTY_QUERY_PARAMS = CollectionUtils2
	...
	enum Type {
		 ...
		 * 
		 * @see <a href="https://www.ietf.org/rfc/rfc3986.txt">RFC 3986,
		 *      appendix A</a>
		 */
	...
	private static class UriTemplateEncoder implements BiFunction<String, Type, String> {
		...
		public String apply(String source, Type type) {
			...
			// Mixed literal parts and URI variables, maybe (encode literal
			// parts only)..
			int level = 0;
	...
	static final class FullPathComponent implements PathComponent {
		...
		@Override
		public List<String> getPathSegments() {
			String[] segments = StringUtils2.tokenizeToStringArray(getPath(), PATH_DELIMITER_STRING);
	...
	static final class PathSegmentComponent implements PathComponent {
		...
		public PathSegmentComponent(List<String> pathSegments) {
			Assert2.notNull(pathSegments, "List must not be null");
		...
		@Override
		public void copyToUriComponentsBuilder(UriComponentsBuilder builder) {
			builder.pathSegment(StringUtils2.toStringArray(getPathSegments()));
		}
	...
	static final class PathComponentComposite implements PathComponent {
		...
		public PathComponentComposite(List<PathComponent> pathComponents) {
			Assert2.notNull(pathComponents, "PathComponent List must not be null");
